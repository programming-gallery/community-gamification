...
if __name__ == "__main__":
    corpusdir = '/tmp/corpuses/programming'
    #corpusdir = '/tmp/corpuses/baseball_new9'
    modeldir = 'test-model'
    #saveCorpus(corpuspath)
    #saveCorpusByGalleryId(gallerycorpusdir)
    corpuspaths = [os.path.join(corpusdir, f) for f in os.listdir(corpusdir)]
    model = Model(override=False)
    model.load(modeldir)
    model.genSTMMTrainingSet(corpuspaths, 'nlp/STTM/dataset/programming.txt')
    model.genBigartmTrainingSet(corpuspaths, 'nlp/bigartm/test_data/programming.txt')
    #res = model.topicModeling(corpuspaths, num_topics=5)
    #for d in [i[:10] for i in res]:
    #    print(d)
